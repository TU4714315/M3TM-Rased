...
function compactEventRows(items: IntelligenceNewsItem[], empty = 'لا توجد أخبار بعد. اضغط "تهيئة المصادر" ثم "جلب الأخبار الآن".'): string {
  return items.slice(0, 6).map((item) => `
    <a class="ops-event-row" href="#/news" aria-label="${escapeHtml(item.title)}">
      <time>${formatDate(item.publishedAt).split('،')[0]}</time>
      <span class="risk-badge ${scoreClass(item.score)}">${escapeHtml(item.risk_level || (item.score >= 65 ? 'مرتفع' : 'متوسط'))}</span>
      <b>${escapeHtml(item.title)}</b>
      <small>${escapeHtml(item.source || providerLabel(item.provider))}</small>
    </a>
  `).join('') || `<p class="empty mini-empty">${empty}</p>`;
}

function compactIntelList(items: IntelligenceNewsItem[], empty = 'لا توجد عناصر مطابقة حالياً.'): string {
  return items.slice(0, 5).map((item) => `
    <a class="compact-intel-row" href="#/news" aria-label="${escapeHtml(item.title)}">
      <span class="risk-dot ${scoreClass(item.score)}"></span>
      <b>${escapeHtml(item.title)}</b>
      <small>${escapeHtml(item.source)} · ${escapeHtml(item.country || item.region || 'إقليمي')}</small>
      <em>${escapeHtml(item.risk_level || 'متوسط')}</em>
    </a>
  `).join('') || `<p class="empty mini-empty">${empty}</p>`;
}

function riskDrivers(news: IntelligenceNewsItem[], grey: GreyIntelligenceItem[], alerts: IntelligenceAlert[]): string[] {
  const drivers = [
    news.some((item) => ['إيران', 'الخليج', 'الأمن الخليجي'].includes(item.category)) ? 'تصاعد مؤشرات مرتبطة بالخليج وإيران.' : '',
    news.some((item) => ['الضربات والهجمات', 'التصعيد العسكري', 'الصواريخ', 'الطائرات المسيرة'].includes(item.category)) ? 'رصد نشاط ضربات أو تهديدات عسكرية ضمن الأخبار.' : '',
    grey.length ? 'وجود مؤشرات رمادية عامة تحتاج مراجعة تحليلية.' : '',
    alerts.some((item) => !item.read) ? 'تنبيهات غير مقروءة تتطلب متابعة تشغيلية.' : '',
    news.some((item) => item.category === 'الأمن السيبراني' || `${item.title} ${item.summary_ar || item.summary}`.includes('اختراق')) ? 'ظهور مؤشرات أمن سيبراني أو اختراق ضمن الرصد.' : '',
  ].filter(Boolean);
  return drivers.length ? drivers : ['لا توجد مؤشرات خطر كافية بعد. ابدأ بتهيئة المصادر ثم جلب الأخبار.'];
}

function greyCompactPanel(items: GreyIntelligenceItem[]): string {
  return `
    <article class="command-panel grey-command-panel compact-grey-panel">
      <div class="command-panel-heading"><h3>المصادر الرمادية</h3><a href="#/grey-intel">عرض الكل</a></div>
      <p class="grey-legal">مؤشرات فقط — لا يتم تخزين بيانات مسربة خام أو كلمات مرور.</p>
      <div class="command-list">
        ${items.slice(0, 4).map((item) => `
          <a class="compact-intel-row" href="#/grey-intel" aria-label="${escapeHtml(item.title)}">
            <span class="risk-dot ${item.risk_level === 'حرج' ? 'critical' : item.risk_level === 'مرتفع' ? 'high' : 'medium'}"></span>
            <b>${escapeHtml(item.title)}</b>
            <small>${escapeHtml(sourceTypeLabel(item.source_type))} · ${escapeHtml(item.data_sensitivity)}</small>
            <em>${escapeHtml(item.risk_level)}</em>
          </a>
...
function repositoryCompactPanel(items: RepositoryIntelligenceItem[]): string {
  const saved = items.filter((item) => item.saved).length;
  const high = items.filter((item) => item.score >= 70).length;
  return `
    <article class="command-panel repo-intel-panel">
      <div class="command-panel-heading"><h3>ذكاء المستودعات</h3><a href="#/repositories/intelligence">عرض الكل</a></div>
      <div class="repo-mini-stats">
        <span><b>${items.length}</b><small>مستودعات</small></span>
        <span><b>${high}</b><small>مرتفعة الصلة</small></span>
        <span><b>${saved}</b><small>محفوظة</small></span>
      </div>
      <div class="command-list">
        ${items.slice(0, 3).map((item) => `
          <a class="compact-intel-row" href="#/repositories/intelligence">
            <span class="risk-dot ${scoreClass(item.score)}"></span>
            <b>${escapeHtml(item.fullName)}</b>
            <small>${escapeHtml(item.language || 'غير محدد')} · ★ ${item.stars}</small>
            <em>${item.score}</em>
          </a>
...
export function renderCommandCenter(
  ...
  const cyber = news.filter((item) =>
    item.category === 'الأمن السيبراني'
    ...
    <section class="command-center command-center-v3" data-ui-version="M3TM_UI_VERSION=command-center-v3">
      ...
      <section class="ops-dashboard-grid">
        <aside class="ops-left-rail">
          <article class="command-panel risk-drivers-panel">
            <div class="command-panel-heading"><h3>لماذا ارتفع الخطر؟</h3><span>${risk.label}</span></div>
            <ul>${drivers.map((item) => `<li>${escapeHtml(item)}</li>`).join('')}</ul>
            <a class="button secondary compact" href="#/reports">عرض التحليل الكامل</a>
          </article>
          <article class="command-panel gulf-brief-panel">
            <div class="command-panel-heading"><h3>الخليج وإيران</h3><a href="#/news">عرض الكل</a></div>
            <div class="regional-mini-map" aria-hidden="true"></div>
            <div class="command-list">${compactIntelList(gulfIran, 'لا توجد مؤشرات خليج/إيران حالياً.')}</div>
          </article>
          <article class="command-panel strikes-summary-panel">
            <div class="command-panel-heading"><h3>الضربات والهجمات</h3><a href="#/news">عرض الكل</a></div>
            <div class="strike-metrics">
              <span><b>${todayCount(strikes)}</b><small>اليوم</small></span>
              <span><b>${strikes.length}</b><small>مرصودة</small></span>
              <span><b>${strikes.filter((item) => item.score >= 65).length}</b><small>مرتفعة</small></span>
            </div>
            <div class="command-list">${compactIntelList(strikes, 'لا توجد ضربات أو هجمات مطابقة حالياً.')}</div>
          </article>
        </aside>

        <main class="ops-main-stage">
          <section class="map-trend-grid">
            ...
            <article class="command-panel threat-timeline-card">
              ...
              <div class="timeline-list">${compactEventRows(news.slice(0, 3), 'لا توجد أحداث في الخط الزمني بعد.')}</div>
            </article>
          </section>

          <article class="command-panel events-table-panel">
            <div class="command-panel-heading"><h3>آخر الأحداث</h3><a href="#/news">عرض جميع الأحداث</a></div>
            <nav class="ops-filter-tabs" aria-label="تصنيفات سريعة"><span>الكل</span><span>أمني</span><span>سياسي</span><span>عسكري</span><span>سيبراني</span></nav>
            <div class="ops-event-table">${compactEventRows(news)}</div>
          </article>
        </main>

        <aside class="ops-right-rail">
          ${greyCompactPanel(state.greyIntel)}
          ${sourceStatusPanel(state)}
          ${repositoryCompactPanel(state.repositories)}
          <article class="command-panel alert-command-panel">
            ...
            <div class="command-list">
              ${unreadAlerts.map((item) => `
                <a class="compact-intel-row" href="#/alerts">
      ...
      <section class="ops-bottom-grid">
        ...
        ${commandSection('الأمن السيبراني', cyber)}
  ...
  view.querySelectorAll<HTMLButtonElement>('[data-command-action]').forEach((button) => {
    button.addEventListener('click', async () => {
      ...
      const busyText: Record<CommandAction, string> = {
        'fetch-arabic': 'جلب الأخبار...',
        'seed-arabic': 'تهيئة المصادر...',
        'seed-grey': 'تهيئة الرمادية...',
        'executive-report': 'إنشاء التقرير...',
      };
      button.disabled = true;
      button.textContent = busyText[action] || 'جاري التنفيذ...';
      try {
